...
def process_row(row, p1_pdb, p1_pid, p2_pdb, p2_pid, pdb_dir):
    """
    ...
    Args:
        ...
        p1_pdb (str): The name of the column containing the first protein PDB ID.
        p1_pid (str): The name of the column containing the first protein Uniprot ID.
        p2_pdb (str): The name of the column containing the second protein PDB ID.
        p2_pid (str): The name of the column containing the second protein Uniprot ID.
        pdb_dir (str): The directory containing the structure files.

    Returns:
        dict or None: A dictionary containing the pair ID and the p-value, or None if the structure files do not exist.
    """
    if not pd.isna(row[p1_pdb]):
        p1 = row[p1_pdb]
    else:
        p1 = row[p1_pid]

    if not pd.isna(row[p2_pdb]):
        p2 = row[p2_pdb]
    else:
        p2 = row[p2_pid]
    ...
    if not os.path.exists(os.path.join(pdb_dir, p1_file)) or not os.path.exists(os.path.join(pdb_dir, p2_file)):
        # Assign None as the p-value instead of dropping the row
        return None

    return compare_fatcat(p1_file, p2_file, pdb_dir, row['pair_id'])

def run_fatcat_dict_job(df, p1_pdb, p1_pid, p2_pdb, p2_pid, pdb_dir, file):
    """
    ...
    Returns:
        str: The path to the output file containing the pair ID and the p-value.
    """
    p_values = []  # List to store the extracted p-values

    # Parallelize the execution of the function using joblib
    p_values = Parallel(n_jobs=-1)(delayed(process_row)(row, p1_pdb, p1_pid, p2_pdb, p2_pid, pdb_dir) for _, row in df.iterrows())
...
#     p_values = run_fatcat_dict_job(df, 'meso_pdb', 'meso_pid', 'thermo_pdb', 'thermo_pid', 'af', 'structure_alig.csv')