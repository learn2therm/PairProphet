...
def make_blast_df(df, mode = 'local'):
    ...
    df: pandas.core.DataFrame
        A 2-column DataFrame containing the query and subject sequences for alignment.
    mode: str
    ...
    df.rename(columns = {original_cols[0]: 'query', original_cols[1]: 'subject'}, inplace = True)
    ...
    blast_df = df.merge(final_df, left_on=['qid', 'sid'], right_on=['query_id', 'subject_id'])
...
def get_matches_gaps(query, subject):
    ...
    n_gaps: int
        Total number of gaps across both aligned sequences.
    n_columns: int
...
def gap_compressed_percent_id(n_matches, n_gaps, n_columns, n_comp_gaps):
    ...
    -------------
    n_matches: int
        Number of matches in match columns
    n_gaps: int
        Number of gaps in match columns
    n_columns: int
        Total number of alignment match columns
    n_compressed_gaps: int
        Number of compressed gaps in match columns
    -------------
    Raises:
    -------------
    Exception:
