'''
This package samples FAFSA training data to represent taxa and protein pairs across the whole distribution of the FAFSA database.

Functions:
    fafsa_sample: Generates a DataFrame containing a random or over/undersampled set from the FAFSA database.
    fafsa_describe: Not supported in initial release. Provides analytics on FAFSA dataset to inform sampling parameters.
    g_prime_i: Calculates g_prime vaues used in fafsa_sample
'''

import numpy as np
import pandas as pd

def fafsa_sample(df, size, stat_samp = 'random', tol = 1e-04):
    '''
    Generates sample DataFrame for downstream processing toward FAFSA training. Option to save analytics showing both sample
    ...
    Raises:
        AttributeError: Necessary features for FAFSA training must exist in input DataFrame.
    '''
    ...
    if False in [i in list(df.columns) for i in features]:
        raise AttributeError("""DataFrame must contain FAFSA features: local_gap_compressed_percent_id,
